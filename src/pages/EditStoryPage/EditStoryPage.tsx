...
const EditStoryPage = () => {
  ...
  useEffect(() => {
    const fetchStory = async () => {
      const res = await http.get(`${STORY_ENDPOINTS.GET_STORY_BY_ID}/${idFromSlug}`);
      setValue("title", res.data.data.title);
      setValue(
        "topicId",
        res.data.data.listTopic.map((topic: { id: any; topicName: any }) => topic.id),
      );
      setTopicValues(
        res.data.data.listTopic.map((topic: { id: any; topicName: any }) => ({
          value: topic.id,
